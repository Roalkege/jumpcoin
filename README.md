...
If your network or ISP blocks the public Tor directory authorities,
Tor gets stuck bootstrapping (you'll see *"Problem bootstrapping.
Stuck at 30% (loading_status)"* in the logs).  In that case, use
**obfs4 bridges**, which route the initial connection through
unlisted relays.  The `:tor` image bundles `obfs4proxy` for this.

1. Get bridges from <https://bridges.torproject.org> (choose the
   **obfs4** transport), or send an email to `bridges@torproject.org`
   with `get transport obfs4` in the body.  You'll receive one or
   more lines like:

   ```
   obfs4 192.0.2.1:443 0123456789ABCDEF0123456789ABCDEF01234567 cert=abc...abc iat-mode=0
   ```

2. Enable bridges via the `TOR_USE_BRIDGES` and `TOR_BRIDGES` env
   vars (one bridge line per line):
   ...
   services:
     jumpcoin-qt:
       ...
       environment:
         ...
         TOR_BRIDGES: |
           obfs4 192.0.2.1:443 0123456789ABCDEF0123456789ABCDEF01234567 cert=abc...abc iat-mode=0
           obfs4 198.51.100.2:9001 FEDCBA9876543210FEDCBA9876543210FEDCBA98 cert=def...def iat-mode=1
       # ... rest unchanged
   ```

   The entrypoint appends `UseBridges 1`, the obfs4
   `ClientTransportPlugin`, and one `Bridge` line per entry to
   `/etc/tor/torrc` at container start.  Leaving `TOR_USE_BRIDGES`
   unset (or `false`) keeps the default behaviour — bridges are a
   purely additive opt-in.

> **No rebuild needed:** Bridges are read from the environment at
> runtime, so swapping them out only requires **restarting the
> container** (`docker compose up -d`), not rebuilding the image.
> Because `torrc` is regenerated from the image on every start, stale
> bridge lines never accumulate.

To confirm it worked, check the Tor log after a start — you should
see `Bootstrapped 100% (done)` instead of it hanging at 30%:

```sh
docker exec jumpcoin-qt tail -30 /var/log/supervisor/tor.log
```
